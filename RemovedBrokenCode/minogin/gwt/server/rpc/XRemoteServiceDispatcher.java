package ru.minogin.gwt.server.rpc;
	...
	protected SerializationPolicy doGetSerializationPolicy(HttpServletRequest request,
		...
		try {
			...
			requestURI = "http://localhost:8080"
					+ requestURI.substring(0, requestURI.lastIndexOf("/") + 1);
			String url = SerializationPolicyLoader.getSerializationPolicyFileName(requestURI
					+ strongName);
			InputStream is = null;
			try {
				is = new URL(url).openStream();
				List<ClassNotFoundException> list = new ArrayList<ClassNotFoundException>();
				serializationPolicy = SerializationPolicyLoader.loadFromStream(is, list);
			}
			catch (Exception e) {
			}
			finally {
