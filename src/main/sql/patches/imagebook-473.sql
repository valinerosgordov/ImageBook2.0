...
SELECT null id,
    11 `type`,
    9 `number`,
    'Планшет горизонтальный — обложка кожзам' `name_ru`,
    `product`.`name_en`,
...
SELECT null id,
    11 `type`,
    10 `number`,
    'планшет буклет 18х18 кожзам' `name_ru`,
    `product`.`name_en`,
    `product`.`availability`,
    `product`.`block_format`,
    `product`.`binding`,
    ...
    `product`.`nonCalc`
FROM `product`
where  type = 11 and number = 3;

-- color ranges
insert into color_range select id, 0, 0 from product where type = 11 and number = 9;
insert into color_range select id, 0, 0 from product where type = 11 and number = 10;

-- albums
insert into album 
SELECT (select id from product where type = 11 and number = 9) id,
    `album`.`size`,
    `album`.`cover_size`,
...
insert into album
SELECT (select id from product where type = 11 and number = 10) id,
    `album`.`size`,
    `album`.`cover_size`,
    `album`.`pdf_cover_width`,
    `album`.`pdf_cover_height`,
    ...
    `album`.`ph_page_lam_price`
FROM `album`
where id = (select id from product where type = 11 and number = 3);

-- cover_lam ranges

insert into cover_lam_range
select (select id from product where type = 11 and number = 9) product_id
     , number
     , _index
  from cover_lam_range where product_id = (select id from product where type = 11 and number = 6);

insert into cover_lam_range
select (select id from product where type = 11 and number = 10) product_id
     , number
     , _index
  from cover_lam_range where product_id = (select id from product where type = 11 and number = 3);
...
select (select id from product where type = 11 and number = 10) product_id
     , number
     , _index
  from page_lam_range where product_id = (select id from product where type = 11 and number = 3);